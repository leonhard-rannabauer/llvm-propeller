...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  // If the input is bit code, disable llvm passes with --save-precodegen as
  // it is already done.  During compilation, do not disable llvm passes with
  // --save-precodegen so that the bc before codegen can be saved.
  if (Input.getType() == types::TY_LLVM_BC &&
      Args.hasArg(options::OPT_save_precodegen) &&
      !C.getDriver().isUsingLTO())
    CmdArgs.push_back("-disable-llvm-passes");
  else if (C.getDriver().isSaveTempsEnabled() &&
           isa<CompileJobAction>(JA)) {
    bool GenOptBC = (C.getDriver().embedBitcodeInObject() ||
        Args.hasArg(options::OPT_save_precodegen));
    if (!GenOptBC || C.getDriver().isUsingLTO())
      CmdArgs.push_back("-disable-llvm-passes");
  }
